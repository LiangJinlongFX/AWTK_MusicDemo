...
static ret_t album_cover_on_paint_self(widget_t* widget, canvas_t* c) {
  ...
  if (album_cover->image == NULL || album_cover->fg_image == NULL ||
      album_cover->bg_image == NULL) {
    return RET_OK;
  }
...
ret_t album_cover_start(widget_t* widget) {
  ...
  widget_create_animator(
      album_cover,
      "cartridge_rotation(to=-3, duration=500, repeat_times=1, yoyo_times=1, easing=sin_out)");
...
ret_t album_cover_pause(widget_t* widget) {
  ...
  widget_create_animator(
      album_cover,
      "cartridge_rotation(to=-45, duration=500, repeat_times=1, yoyo_times=1, easing=sin_out)");
...
static ret_t timer_album_cover(const timer_info_t* timer) {
  ...
  if (album_cover->is_play == TRUE) {
    ...
    if (album_cover->rotation > 360) album_cover->rotation = 0;
 ...
 * 是否可点击
 */
static const char* s_album_cover_clone_properties[] = {
    ALBUM_COVER_PROP_IMAGE,    ALBUM_COVER_PROP_BG_IMAGE, ALBUM_COVER_PROP_FG_IMAGE,
    ALBUM_COVER_PROP_ROTATION, ALBUM_COVER_PROP_ISPLAY,   NULL};

/*
 * 设置控件基本属性
 */
static const widget_vtable_t s_album_cover_vtable = {
    .size = sizeof(album_cover_t),                            //大小
